...
<body>
  ...
  <div class="content_volver">
    <h2 class="text-left mb-4">Control de canastillas buenas</h2>
    <div class="botones_control">
      <a class="volver" href="./control-rotas.php">Canastillas rotas</a>
    ...
    <table class="table">
      ...
      <tbody>
        ...
        while ($rowPunto = $resultPuntosVenta->fetch_assoc()) {
          ...
          $queryExistente = "SELECT normales FROM canastillas_existencia WHERE punto = '$punto'";
          $resultExistente = $conexion->query($queryExistente);
          $rowExistente = $resultExistente->fetch_assoc();
          $existentes = $rowExistente['normales'];

          // Consulta para obtener el valor inicial desde la tabla canastillas_iniciales
          $queryInicial = "SELECT normales FROM canastillas_iniciales WHERE punto = '$punto'";
          $resultInicial = $conexion->query($queryInicial);
          $rowInicial = $resultInicial->fetch_assoc();
          $inicial = $rowInicial['normales'];

          // Consulta para obtener la suma de las entradas desde la tabla canastillas_registros_prueba
          $queryEntradas = "SELECT SUM(cantidad) AS total_entradas FROM canastillas_registros_prueba WHERE punto_recibe = '$punto' AND tipo = 'Normal'";
          ...
          $querySalidas = "SELECT SUM(cantidad) AS total_salidas FROM canastillas_registros_prueba WHERE punto_envia = '$punto' AND tipo = 'Normal'";
